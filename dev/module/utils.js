...
var Utils = {
  ...
  getUniqueValues: function(_array) {
    return _array.filter(function(value, index, self){
  ...
  wrapText: function (text, width, anchor) {//from https://bl.ocks.org/mbostock/7555321
    text.each(function() {
      ...
      anchor = anchor != undefined ? anchor : "start";
      var tspan = text.text(null).append("tspan").attr("text-anchor",anchor).attr("x", 0).attr("y", y).attr("dy", dy + "em");
      ...
      while (word = words.pop()) {
        ...
        if (tspan.node().getComputedTextLength() > width) {
          ...
          tspan.text(line.join(" "));
          line = [word];
          tspan = text.append("tspan").attr("text-anchor",anchor).attr("x", 0).attr("y", y).attr("dy", ++lineNumber * lineHeight + dy + "em").text(word);
        }
      }
    });
  },
